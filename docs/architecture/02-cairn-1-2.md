...
description: What changes for a sole owner's repository once Cairn 1.2 lands, in the sections of the 1.1 page, naming the record behind every statement, ADR-030 to ADR-045, and what 1.2 removes from 1.1.
...
**Amended on 2026-09-25** by the milestone rule `cairn-active` implements,
[ADR-045](../adr/ADR-045-a-milestone-reads-the-paths-it-counts.md),
promoted from the [journal entry](../../project/log/2026-09-26-cp-cairn-016.md)
of CP-CAIRN-016 at blob `fd04ab18b99823efbdffeb34297c5a092ccce973`, which
stays exactly as it was. ADR-031 decision 2's *that path's* gives way for
a milestone; the sentence this amendment adds is marked *since 2026-09-25*.
...
reports a stale cell (ADR-031, decision 2). Since 2026-09-25, a
milestone reads the paths its row names and those in the table under
the heading ending with its short name (ADR-045). A record's
implementation table may say *pending* against a decision no path has landed, naming
...
| `cairn-active` | fills the register's state cells from the records, and reports a stale one | ADR-031 d2; ADR-045 |
