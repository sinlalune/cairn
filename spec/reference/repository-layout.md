...
| `cairn.lock.json` | the release installed here, the commit it was cut from, one digest per kit file as the kit wrote it, so `status` can tell a pristine file from an edited one and `update` can rewrite the first, the host files the repository declined, which `update` never writes and `status` reads as declined, the digests of the host's own content — the live view as generated, the configuration as migrated — which `status` reads as `host`, and the Ponytail version the stance was fetched at, with whether the last run read it; the digests cover the copies of every skill under `.claude/skills/`, where Claude Code loads them | written by the `cairn` command; never hand-edited |
