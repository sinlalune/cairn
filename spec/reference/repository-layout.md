...
| `cairn.lock.json` | the release installed here, the commit it was cut from, one digest per kit file as the kit wrote it, so `status` can tell a pristine file from an edited one and `update` can rewrite the first, and the host files the repository declined, which `update` never writes and `status` reads as declined | written by the `cairn` command; never hand-edited |
