...
| A typed work unit, coherent in one commit | **partially implemented**; `work-unit` requires a `cairn-unit` block for the record's current step with a known type, requires a running record that has completed a unit to name a full object id in its resume checkpoint, and blocks source that changed without a module note and the path record in the same change; the area-precise note is advisory; `review` requires the current unit's ledger to carry a `#### Review` section that is not empty — the fourth movement's record, one line per finding with its disposition — reading the ledger's newest completed unit, the unit under review, with `current_step` selecting nothing (ADR-017 decision 2; ADR-026 decision 2), and excepting the `closure` type, which writes no step file | the type does not key the required parts: on the branch-versus-trunk comparison the changed set is cumulative across every unit on the branch, so keying on the current unit's type would be unsound. `review` reads presence and emptiness and nothing else: whether the reader was a fresh context, and whether the dispositions are honest, is what the owner reads at the candidate — a checker that scored them would be inventing a judgement. On the flat record shape the ledger is the record itself, so one section there answers for every unit of that path; a `cairn-unit` block in the `index.md` of a folder record is not read at all, because a section answering for it could be rewritten the next minute. `current_step` selected the unit until 2026-09-14, over a list sorted by ordinal, so a field left behind could only point the rule at a unit older than the one just written; it stays in the schema, where `work-unit` reads it |
...
| One invocation, one verdict: local and CI agree on one tree | **implemented**; on a path branch the base is the trunk, there is no narrower form, and three fixtures assert both invocations reach one verdict on one real tree; the branch is read from the host only for the repository the host checked out; since 2026-09-14 `comparison` is the predicate behind this row, reporting a base that cannot be compared with the commit under judgement and, off a path branch, one that already contains it | a host that wires its CI to a different command is outside what this can observe |
...
| The trust boundary and the enforcement profile | **printed, not proved**; the checker prints the declared profile and its two transports with every verdict, asks the host nothing and makes no network call (ADR-029, superseding ADR-001 decision 6 and ADR-026 decision 1), and `cairn-init` refuses to install `protected` | that the host requires the check and the approval on the exact commit that lands, and refuses a squash, are host claims no local reader can prove; they are read on the host's own settings page |
...
| **Blocking** | `comparison` | diff | A base was asked for and cannot be compared with this commit — it does not resolve, or shares no history with it — or, off a path branch, it already contains this commit, so every changed-file rule was narrowed to the working tree. GitHub's sentinel for a branch's first push is advisory, not blocking: nothing precedes that commit | `refExists(flag) && mergeBase(base, HEAD) !== null, else unresolvable; !onPath && mergeBase(base, HEAD) === HEAD => empty; /^0{40,}$/ => advisory. Reported inconclusive, never fatal` |
| **Blocking** | `concept-orphan` | corpus | A concept note that no normative or learning text outside the wiki links to. The root is read RECURSIVELY, through every folder it holds (ADR-011 d2), and a note is named by its path under the root, so a link must reach the folder the note is in | `orphanConcepts(walk(concepts root) relative to it, conceptLinkTargets(documents outside the concepts folder))` |
...
| **Blocking** | `review` | diff | The ledger of a path's current unit — its step record, or the flat record that is one — carries no `#### Review` section, or carries it empty; the `closure` type, which writes no step file, is excepted (ADR-017 d2) | `reviewSection(the current unit's ledger) — the ledger's NEWEST completed unit, the unit under review, `current_step` selecting nothing (ADR-026 d2); presence and emptiness only, and never the `index.md` of a folder record; the findings and their dispositions are the owner's to read at the candidate` |
...
| **Blocking** | `transition` | diff | Changed path state is not an allowed lifecycle transition, a path branch claims done, a declaration was deleted rather than archived, or the prior state is unavailable. A range that holds the merge as well reads what the record declared in a commit immediately behind the arrival — any parent of it, a merge having two and an octopus more (ADR-027) — rather than at the base (ADR-008 d2) | `transitionErrors(previous, current, onPathBranch, integrationState(record, comparisonRef, id).readyBehind), readyBehind = (commit ? parents : [HEAD]).some(statusAt === ready)` |
...
| *Advisory* | `comparison` | diff | A base was asked for and cannot be compared with this commit — it does not resolve, or shares no history with it — or, off a path branch, it already contains this commit, so every changed-file rule was narrowed to the working tree. GitHub's sentinel for a branch's first push is advisory, not blocking: nothing precedes that commit | `refExists(flag) && mergeBase(base, HEAD) !== null, else unresolvable; !onPath && mergeBase(base, HEAD) === HEAD => empty; /^0{40,}$/ => advisory. Reported inconclusive, never fatal` |
| *Advisory* | `concept-growth` | corpus | A change adds concept articles anywhere under the root, its folders included; reported so vocabulary growth is a visible decision | `addedConcepts(ls-tree -r listing at previousRef, current recursive listing), both relative to the concepts root, diff-scoped to it` |
