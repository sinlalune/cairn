...
description: Every ask the five adopter notes make of 1.1, one checkbox line each, grouped by theme and de-duplicated, derived on 2026-09-06 from the owner's answers to the decisions page: thirty-seven taken, two refused; the owner ticks what 1.1 takes, writes "later" or "no" beside what it does not, and the promotion path that follows is bound to this note.
...
[decisions page](./2026-09-06-cairn-1-1-decisions.md) on 2026-09-06; each
technical line below is ticked or marked `no` from that answer, with the
derived reading quoted under it where the answer picks a variant. A ticked
box is a ruling for 1.1. Two lines are refused. R39 was added by the
owner's fourth removal. Nothing here is undecided.
...
- [x] **R01 — What a writer may do while a registration request is open.**
  ...
  > Q1 and Q19: the sole owner's go-ahead is given in the chat and recorded as the opening acceptance; the plan is landed on the trunk directly (`transport.registration: manual-git`), no `register/` request, no `registration-pending` rule; implementation waits for that landing.
- [x] **R02 — The registration request is the opening review.** Name it;
  an amendment after that review is a superseding acceptance block. *(A;
  Crumbz did it five times unprompted.)*
  > Q2: the owner's reading of the plan before it lands is the named step; an amendment is a superseding acceptance block.
- [ ] **R03 — The solo-owner closing acceptance has a stated shape.** When
  ...
  recorded approval.)*
  > `no` — Q4: the merge click is the acceptance; no solo-owner shape is added.
- [x] **R04 — An owner's acceptance test before merge.** A named step in
  the closure: the owner tries the candidate and records it in the request
  as a checkbox only the owner ticks. *(O, A.)*
  > Q3: always, for every path, a checkbox in the request that only the owner ticks.
- [x] **R05 — Merge only after the request's run on the `ready` commit is
  read green.** One sentence in the close skill's step 5. *(G, A; PR #2 and
  PR #41 merged before their check ran.)*
- [x] **R06 — The `ci` profile says what it cannot see.** With a forge
...
- [x] **R07 — One gate per candidate.** The workflow runs once per commit
  ...
  *(K.)*
  > Q19: one run per commit that can land; the push run on `path/**` goes.
- [x] **R08 — `running → done` on the trunk is refused when no `ready`
...
- [x] **R09 — `registration-base` reads the transition, not the file
...
- [x] **R10 — `scope-digest` is judged at every transition on every ref.**
  ...
  both runs green.)*
  > Q7: judged at every transition on every ref, checkboxes kept.
- [ ] **R11 — The checkboxes leave the definition of done.** The template
  ...
  *(C, K, A; ticked on three paths by two agents in three days.)*
  > `no` — Q7 and Q19: the checkboxes stay; R10 catches every tick instead.
- [x] **R12 — Two live paths with intersecting `writes:` is a finding.**
  ...
  > Q8: advisory at registration and every unit, and the later path declares `depends_on` or the owner records the accepted race.
- [x] **R13 — A running path with an unpinned checkpoint after its first
  unit is refused.** *(G; fifteen Gemini units left it `unpinned`.)*
- [x] **R14 — Range rules are path-scoped.** A rule over `base..candidate`
  reads only this path's records as evidence about this path; every range
  walker audited against that sentence. *(K.)*
- [x] **R15 — Every blocking rule's fixture contains a merged trunk commit
  carrying another path's completed unit.** *(K.)*
- [x] **R16 — The checker resolves the path branch from where it stands.**
  Local ref, else HEAD on a detached request head, else `origin/<branch>`;
  every rule that names the branch audited. *(K.)*
- [x] **R17 — The three Crumbz repairs come upstream.** Same-branch step
...
- [x] **R22 — The journal template loses its duplicate `path:` key.**
  *(G, A; copied forward into all fifteen Crumbz entries.)*
- [x] **R38 — The roadmap register becomes reportable.** `cairn-active`
  reports a register still carrying the installer's row. *(G, A; sixteen
  paths, placeholder row untouched.)*
- [x] **R23 — Transport branches are deleted on merge; `path/*` stays until
  the path is archived.** *(O; forty-five branches on Crumbz.)*
- [x] **R24 — Closure cleanup gets a predicate or a hand-off.** The close
  skill's step 6 runs before the report, or `cairn-active` lists worktrees
  whose path is `done`. *(A; seven stale worktrees.)*
  > Q11: the agent runs the close skill's step 6 itself and reports a failure to remove.

## 4. What the skills say

- [x] **R25 — `repair` is defined where the type is chosen.** One sentence
  in the unit skill's type table: a repair corrects a protocol violation
  and names it. *(G, C; eleven misuses in one day.)*
- [x] **R26 — The unit plan names the definition-of-done item it
  advances.** *(G; CP-002 outgrew its outcome by six units.)*
- [x] **R27 — No object id is typed by hand.** The open skill gives
  `base_commit` the same rule as the digest. *(G.)*
- [x] **R28 — Refusals name the remedy.** Every blocking message says what
  ...
  named.)*
- [x] **R29 — The unit skill states the worktree precondition.** Install
  dependencies before the first gate; the reference example reads the exit
  code, not the output. *(K.)*
- [x] **R30 — A module note has a split trigger.** An area whose note every
...
- [x] **R32 — Promotion produces a page a new reader can read first.** A
  ...
  > Q16: one readable page per product surface, promotion must write or update it.
- [x] **R33 — The kit installs a local pointer to the protocol.** One
...
- [x] **R36 — `cairn-postmortem`.** A command that prints, for one path or
  ...
  > Q18: triggered by a red run in CI; the reading half only.
- [x] **R37 — The kit's self-test is not `npm test`.** A `cairn-test`
...
- [x] **R39 — The module note describes the current state only.** No
  per-step history under "Current State"; the journal holds it. *(O, A;
  Q19 fourth item.)*
