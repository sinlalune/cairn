...
- [ ] **Yes, right away.** The agent may start coding on its own branch as
  soon as the plan is submitted; your review of the plan comes when you get
  to it, and the work cannot be finished until you have approved the plan.
- [ ] **Yes, and skip the request.** You are the only owner, so the agent
  records the plan directly on the main line without a pull request; you
  review the plan with the finished work instead.
- [ ] **No.** The agent stops and tells you it is waiting for your
  go-ahead, and does nothing on that path until you give it.
...
- [ ] **Yes.** "The owner reviews the plan" is a named step; the agent
  expects it and records your changes as an amendment.
- [ ] **No.** Keep it informal.
...
- [ ] **Always.** Before the merge, you open the app and try the change,
  and you tick a box in the pull request that only you may tick.
- [ ] **When it is visible.** Only for changes to screens or behaviour you
  can see; pure back-end or documentation work goes on tests alone.
- [ ] **Never.** Tests and the agent's checks are enough.
...
- [ ] **A line you write.** Before merging, you write one line in the
  request yourself ("accepted, Toure, date"); the merge without that line is
  a mistake the tool reports.
- [ ] **A merge from your terminal.** You integrate from your own machine
  with a command instead of GitHub's button, and that command records the
  acceptance.
- [ ] **The merge click is enough.** Keep it as it is.
...
- [ ] **Yes.** The closing instructions say "merge only after the check on
  the final commit is green", and the tool tells you when GitHub is not
  enforcing it.
- [ ] **No.** You will watch for it yourself.
...
- [ ] **No.** The tool refuses; the sequence must be followed.
- [ ] **Yes, with a warning.** Allowed, but reported.
...
- [ ] **Remove the checkboxes.** Outcomes are a plain list; completion is
  stated in the closing record and the journal.
- [ ] **Keep them, and catch every tick.** The tool checks the seal at every
  stage, including the last one.
...
- [ ] **Warn and require a choice.** At the start, the tool reports the
  overlap; the later path either declares it waits for the earlier one, or
  you write that you accept the race.
- [ ] **Warn only.**
- [ ] **Nothing.** Overlap is the writer's judgement.
...
- [ ] **The agent removes it** as the last step of closing, and reports if
  it could not.
- [ ] **The agent lists them** and you remove them.
...
- [ ] **Move it out.** The agent puts such blocks in a separate file the
  kit does not own, and the entry file points at it.
- [ ] **Ignore it.** The block is never committed.
- [ ] **Accept it** in the entry file.
...
- [ ] **Yes.** Define "repair" where the type is chosen, make each unit
  name the outcome it advances, and forbid hand-typed ids.
- [ ] **Later.**
...
- [ ] **Yes, split by main component**, and the agent proposes the split
  when one note is being touched by every path.
- [ ] **No**, one note per application is fine.
...
- [ ] **One product page.** Every promotion path must write or update a
  single readable page about the product, using the concept notes as its
  glossary.
- [ ] **One page per product surface** (board, match lab, settled…).
- [ ] **No.** Architecture and decisions are enough.
...
- [ ] **Yes.** The kit installs a small `cairn/` folder with one page that
  names the release, links the chapters and the skills, and lists the files
  the kit owns.
- [ ] **No.** The links are enough.
...
- [ ] **On demand only.** You or the agent asks for it.
...
line above. After you answer, the rulings note is ticked, deferred or
